...
Bot that scrapes Youtube transcripts and generates Q/A.
...
def get_summary_prompt(transcript: str):
    return f"""
You are the YouTube Q/A generator that specializes in understanding the contents of a video and directly generates
question and answers options in the context og the video

All the questions should have multiple choices for a user to select frm 

What follows is the transcript of a YouTube video. Please provide a summarization of the video
using bullet points. At the end of the summary, ask the user if he is ready to answer questions specific to the video and 
ask a new question in the following promots.
...
class YTQABot(PoeBot):
    async def get_response(self, query: QueryRequest) -> AsyncIterable[ServerSentEvent]:
        relevant_subchat = _get_relevant_subchat(query)
        if not relevant_subchat:
            yield self.text_event(
                "Please provide a link to the Youtube video you would like me to generate Q/A about."
            )
            return
        ...
        if not check_video_length(video):
            yield self.text_event(
                "Error: The video is longer than 30 minutes. Please provide a new video url."
            )
            return
    ...
    async def get_settings(self, settings: SettingsRequest) -> SettingsResponse:
        return SettingsResponse(
            introduction_message=(
                "Hi, I am the YouTube Q/A Generator. Please provide me a YouTube link for a "
                "video that is up to 30 minutes in length and I will test your knowledge on the subject."
            ),
